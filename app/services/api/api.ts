...
export class Api {
  ...
  async consignmentSearchByNumber(
    authorization: string,
    consignmentRequest: any,
  ): Promise<Types.ConsignmentResult> {
    const response: ApiResponse<any> = await this.apisauce.post(
      "",
      convertToXMLRequest(CONSIGNMENT_SEARCH, consignmentRequest, "requests"),
      { headers: { Authorization: "Basic " + authorization } },
    );
    if (!response.ok) {
      const problem = getGeneralApiProblem(response);
  ...
  async saveConsignment(
    ...
    const response: ApiResponse<any> = await this.apisauce.post(
      "ccsLocal/statusUpdateXML",
      convertToXMLRequest(SAVE_CONSIGNMENT, consignmentRequest, "statusUpdates"),
      { headers: { Authorization: "Basic " + authorization } },
    );
    if (!response.ok) {
      const problem = getGeneralApiProblem(response);
  ...
  async getTownAPI(townPostalCode: any): Promise<Types.GetTownResult> {
    const response: ApiResponse<any> = await this.apisauce.get(
      `https://maps.googleapis.com/maps/api/geocode/json?key=AIzaSyBFxYi9_fjIKVcmFOv00zejI8pks_TnzBw&address=${townPostalCode}&components=country:AU&sensor=true`,
    );
    if (!response.ok) {
      const problem = getGeneralApiProblem(response);
      if (problem) return problem;
  ...
  async getCurrentLocation(latitude: any, longitude: any): Promise<Types.GetLocationResult> {
    const response: ApiResponse<any> = await this.apisauce.get(
      `https://maps.googleapis.com/maps/api/geocode/json?address=${latitude +
        "," +
        longitude}&key=AIzaSyBFxYi9_fjIKVcmFOv00zejI8pks_TnzBw`,
    );
    if (!response.ok) {
      const problem = getGeneralApiProblem(response);
      if (problem) return problem;
  ...
  async getList(authorization: string, getListRequest: any): Promise<Types.GetListResult> {
    const response: ApiResponse<any> = await this.apisauce.post(
      "",
      convertToXMLRequest(CONSIGNMENT_SEARCH, getListRequest, "requests"),
      { headers: { Authorization: "Basic " + authorization } },
    );
    if (!response.ok) {
      const problem = getGeneralApiProblem(response);
  ...
  async getACalculatedRate(
    authorization: string,
    getARateRequest: any,
  ): Promise<Types.GetARateResult> {
    const response: ApiResponse<any> = await this.apisauce.post(
      "",
      convertToXMLRequest(CONSIGNMENT_SEARCH, getARateRequest, "requests"),
      { headers: { Authorization: "Basic " + authorization } },
    );
    if (!response.ok) {
      const problem = getGeneralApiProblem(response);
