...
export interface ComMaturityListProps {
...
export const ComMaturityList: FunctionComponent<ComMaturityListProps> = observer(props => {
  const { item, index } = props;
  const { authStore } = useStores();
  const Maturitys = item;
  return (
    <View key={index} style={FLAT_LIST}>
      {!authStore.IsTrader && (
        <View style={FLAT_LIST_VIEW}>
          <Text style={ITEM_LABEL} tx="common.FullName" />
          <Text style={TEXT_VALUE} text={Maturitys.FullName ? Maturitys.FullName : " "} />
        </View>
      )}
      <View style={FLAT_LIST_VIEW}>
        <Text
          extraText={":"}
          tx={"MaturityList.MaturityCode"}
          style={ITEM_LABEL}
          preset={"normal"}
        />
        <Text style={FormNo_Value} preset={"normal"}>
          {Maturitys.MaturityCode}
        </Text>
      </View>
      ...
      <View style={Amount_Curr_VIEW}>
        <Text
          extraText={":"}
          tx={"MaturityList.FormCode"}
          style={Amount_Curr_LABEL}
          preset={"normal"}
        />
        <Text
          extraText={":"}
          tx={"MaturityList.Amount"}
          style={Amount_Curr_LABEL}
          preset={"normal"}
        />
      </View>
      ...
      <View style={Amount_Curr_VIEW}>
        ...
        <Text style={Amount_Curr_VALUE} preset={"normal"}>
          {currencyFormatUSD(parseFloat(Maturitys.Amount))}
        </Text>
      </View>
      ...
      <View style={Bank_Branch_VIEW}>
        <Text
          extraText={":"}
          tx={"MaturityList.MaturityDate"}
          style={Bank_Branch_LABEL}
          preset={"normal"}
        />
        <Text
          extraText={":"}
          tx={"MaturityList.MaturityRemaining"}
          style={Bank_Branch_LABEL}
          preset={"normal"}
        />
      </View>
      ...
      <View style={Bank_Branch_VIEW}>
        <Text style={Bank_Branch_VALUE} preset={"normal"}>
          {Maturitys.MaturityDate[0].substring(0, 10)}
        </Text>
        <Text style={Amount_Curr_VALUE} preset={"normal"}>
          {currencyFormatUSD(parseFloat(Maturitys.MaturityRemaining))}
        </Text>
      </View>
      ...
      <View style={Form_Status_View}>
        <Text
          extraText={":"}
          tx={"MaturityList.FlowStatusNameIM"}
          style={Form_Status_LABEL}
          preset={"normal"}
        />
        <Text style={Form_Status_VALUE} preset={"normal"}>
          {Maturitys.FlowStatusNameIM}
        </Text>
      </View>

      {/* <View style={FROM_TO_VIEW}>
        <View style={WEIGHT_VIEW}>
          <Text extraText={":"} tx={"MaturityList.wgt"} style={WEIGHT_LABEL} preset={"normal"} />
        ...
        <View style={VOLUME_VIEW}>
          <Text extraText={":"} tx={"MaturityList.vol"} style={WEIGHT_LABEL} preset={"normal"} />
          <Text style={TEXT_VALUE} preset={"normal"}>
            {cons.totalVolume}
          </Text>
        </View>
