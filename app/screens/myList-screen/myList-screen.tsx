...
export const MyList: FunctionComponent<MyListProps> = observer(props => {
    ...
    if (status) {
      const deliveredArray = filterListData.filter(value => {
        ...
        if (status == "UNDELIVERED") {
          ...
          let currentStatus =
            value.freightStateHistory[value.freightStateHistory.length - 1].status[0];
          let currentStatusDate = value.freightStateHistory[
            value.freightStateHistory.length - 1
  ...
  const getListApi = async () => {
    ...
    if (myListStore.responseSuccess) {
      let i = 0;
      const arr = myListStore.getListData;
      let statusDataArr = [
        { label: "ALL", value: "ALL" },
        { label: "UNDELIVERED TODAY", value: "UNDELIVERED" },
      ];
      let tempArr = [];
  ...
  return (
    ...
    >
      ...
      <MenuButton title={"myList.header"} onPress={handleDrawer} />
