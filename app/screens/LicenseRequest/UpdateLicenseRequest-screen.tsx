...
let isBank = true;
export const UpdateLicenseRequest: FunctionComponent<UpdateLicenseRequest> = observer(props => {
  ...
  const [ImporterName, onImporterName] = useState(LicenseRequest.ImporterName[0]);

  const [LicenseRequestCode, onLicenseRequestCode] = useState(LicenseRequest.LicenseRequestCode[0]);

  const [isValidImporterAddress, setValidImporterAddress] = useState(true);
  const [ImporterAddress, onImporterAddress] = useState(LicenseRequest.ImporterAddress[0]);
  ...
  const [ArrivalPort, onArrivalPort] = useState(LicenseRequest.ArrivalPort[0]);

  const [isValidTotalValue, setValidTotalValue] = useState(true);
  const [TotalValue, onTotalValue] = useState(LicenseRequest.TotalValue[0]);
  ...
  const [Bank, onBank] = useState(+LicenseRequest.Bank[0] - 1);
  ...
  const [Purpose, onPurpose] = useState(+LicenseRequest.Purpose[0]);
  ...
  const [Notes, onNotes] = useState(LicenseRequest.Notes[0]);
  ...
  const onSave = async () => {
    ...
    let Licenseobject =
      "{LicenseRequestCode:" +
      LicenseRequest.LicenseRequestCode[0] +
      ",LoadingPort:" +
      LoadingPort +
      ",Destination:" +
      Destination +
      ...
      "'}";
    LicenseRequestStore.setUpdateLicenseObject(Licenseobject);
    LookupStore.GetCommodity(authStore.authorization);
    LicenseRequestStore.setLicenseRequestCommodityDetail(LicenseRequestStore.getCommodityListData);

    if (BankBranch !== 0) {
      props.navigation.navigate("UpdateLicenseRequestCommodity");
    } else {
      showAlert("LicenseRequest.enterBankBranch");
    }
  };
              ...
              <View style={LicenseRequest_VIEW}>
                <View style={TextFiled_VIEW}>
                  <View style={DETAIL_VIEW}>
                    <Text tx={"LicenseRequest.requestcode"} style={[ITEM_LABEL]} />
                  </View>
                  <TextField
                    inputStyle={TEXT_VALUE}
                    errorTx={isValidImporterName ? undefined : "LicenseRequest.requestcode"}
                    editable={false}
                    defaultValue={LicenseRequest.LicenseRequestCode}
                ...
                <View style={TextFiled_VIEW}>
                  <TextField
                    ...
                    defaultValue={LicenseRequest.ImporterName[0]}
                    value={ImporterName}
                  />
                </View>
                <View style={DETAIL_VIEW}>
                ...
                <View style={TextFiled_VIEW}>
                  <TextField
                    ...
                    defaultValue={LicenseRequest.ImporterAddress}
                ...
                <SafeAreaView style={TextFiled_VIEW}>
                  <SearchableDropdown
                    ...
                    defaultIndex={LicenseRequest.Custom_UnitPK[0]}
                ...
                <SafeAreaView style={TextFiled_VIEW}>
                  <SearchableDropdown
                    ...
                    //mapping of item array
                    defaultIndex={LicenseRequest.CountryCode}
                    //default selected item index
                    placeholder="Select Value"
                    //place holder for the search input
                    resetValue={false}
                ...
                <View style={TextFiled_VIEW}>
                  <TextField
                    ...
                    defaultValue={LicenseRequest.TotalValue}
                    value={TotalValue}
                  />
                </View>
                ...
                <SafeAreaView style={TextFiled_VIEW}>
                  <SearchableDropdown
                    ...
                    //mapping of item array
                    defaultIndex={+LicenseRequest.CurrencyCode[0]}
                    //default selected item index
                    placeholder="Select Value"
                    //place holder for the search input
                    resetValue={false}
                ...
                <View style={TextFiled_VIEW}>
                  <IssueText
                    ...
                    defaultValue={LicenseRequest.Notes}
                    value={Notes}
                  />
                </View>
