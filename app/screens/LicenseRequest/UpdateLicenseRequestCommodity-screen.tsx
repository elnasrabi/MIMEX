...
export interface UpdateLicenseRequestCommodity {
...
let isBank = true;
export const UpdateLicenseRequestCommodity: FunctionComponent<UpdateLicenseRequestCommodity> = observer(
  props => {
    const isFocused = useIsFocused();
    const { LicenseRequestStore, authStore, LookupStore } = useStores();
    const LicenseRequestCommodity = LicenseRequestStore.getCommodityListData;

    let CommodityArry = [];

    let i = 0;
    for (i = 0; i < LicenseRequestCommodity.length; i++) {
      let statusIsPresent = _.indexOf(
        CommodityArry,
        LicenseRequestCommodity[i].CommodityNameAR[0],
        0,
      );
      if (statusIsPresent == -1) {
        CommodityArry.push({
          CommodityNameAR: LicenseRequestCommodity[i].CommodityNameAR[0],
          CommodityCode: LicenseRequestCommodity[i].CommodityCode[0],
          Qty: LicenseRequestCommodity[i].Qty[0],
          UnitPrice: LicenseRequestCommodity[i].UnitPrice[0],
        });
      }
    }
    ...
    const onSave = async () => {
      ...
      if (CommoditytoPost.length > 0) {
        LicenseRequestStore.setUpdateLicenseCommodityObj(CommoditytoPost);

        LicenseRequestStore.UpdateLicenseRequest(
          LicenseRequestStore.UpdateLicenseObj,
          JSON.stringify(LicenseRequestStore.UpdateLicenseCommodityObj),
        );
