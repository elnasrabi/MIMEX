...
export interface UpdateOperationRequestPaymentMethod {
...
let isBank = true;
export const UpdateOperationRequestPaymentMethod: FunctionComponent<UpdateOperationRequestPaymentMethod> = observer(
  props => {
    const isFocused = useIsFocused();
    const { OperationRequestStore, authStore, LookupStore } = useStores();

    const OperationRequestPM = OperationRequestStore.getPaymentMethodListData;

    let PaymentMethodArry = [];

    let i = 0;
    for (i = 0; i < OperationRequestPM.length; i++) {
      let statusIsPresent = _.indexOf(
        PaymentMethodArry,
        OperationRequestPM[i].PaymentMethodNameAR[0],
        0,
      );
      if (statusIsPresent == -1) {
        PaymentMethodArry.push({
          PaymentMethodNameAR: OperationRequestPM[i].PaymentMethodNameAR[0],
          PaymentMethod: OperationRequestPM[i].PaymentMethodCode[0],
          Percentage: OperationRequestPM[i].Percentage[0],
        });
      }
    }
    ...
    const onSave = async () => {
      ...
      if (prcnt === 100) {
        ...
        console.log(
          ...
          JSON.stringify(OperationRequestStore.UpdateOperationCommodityObj),
        );

        OperationRequestStore.UpdateOperationRequest(
          OperationRequestStore.UpdateOperationObj,
          JSON.stringify(OperationRequestStore.UpdateOperationCommodityObj),
