...
let isBank = true;
export const UpdateOperationRequest: FunctionComponent<UpdateOperationRequest> = observer(props => {
  ...
  const [ExporterName, onExporterName] = useState(OperationRequest.ExporterName[0]);

  const [isValidDocArrivalDate, setValidDocArrivalDate] = useState(true);
  const [DocArrivalDate, onDocArrivalDate] = useState(new Date(OperationRequest.DocArrivalDate[0]));
  const [show, setShow] = useState(false);

  const [isValidImportPurpose, setValidImportPurpose] = useState(true);
  const [ImportPurpose, onImportPurpose] = useState(OperationRequest.ImportPurpose[0]);

  const [isValidExporterCountry, setValidExporterCountry] = useState(true);
  const [ExporterCountry, onExporterCountry] = useState(+OperationRequest.CountryCode[0] - 1);
  ...
  const [TotalValue, onTotalValue] = useState(OperationRequest.TotalValue[0]);

  const [isValidCurrency, setValidCurrency] = useState(true);
  const [Currency, onCurrency] = useState(+OperationRequest.CurrencyCode[0] - 1);

  const [isValidBank, setValidBank] = useState(true);
  const [Bank, onBank] = useState(+OperationRequest.Bank[0]);

  const [isValidBankBranch, setValidBankBranch] = useState(true);
  const [BankBranch, onBankBranch] = useState(+OperationRequest.BranchCode[0] - 1);

  const [BankBranchList, onBankBranchList] = useState([]);

  const [isValidNotes, setValidNotes] = useState(true);
  const [Notes, onNotes] = useState(OperationRequest.Notes[0]);
  ...
  const addElement = () => {
    let UpdateArray = [...initialElements, { id: idx, text: "Object " + (idx + 1) }];
    incr(idx + 1);
    console.log(initialElements.length);
    setExampleState(UpdateArray);
    changeEl(UpdateArray);
  };
  ...
  const onSave = async () => {
    ...
    let Operationobject =
      ...
      OperationRequest.OperationRequestCode[0] +
      ",ImportPurpose:" +
      ImportPurpose +
      ",ExporterCountry:" +
      ExporterCountry +
      ...
      "'}";
    OperationRequestStore.setUpdateOperationObject(Operationobject);
    LookupStore.GetCommodity(authStore.authorization);
    OperationRequestStore.setUpdateOperationCommodityObj(Operationobject);
    LookupStore.GetCommodity(authStore.authorization);
    OperationRequestStore.setOperationRequestCommodityDetail(
      OperationRequestStore.getCommodityListData,
    );

    if (BankBranch !== 0) {
      console.log(Bank + "--" + BankBranch);
      props.navigation.navigate("UpdateOperationRequestCommodity");
    } else {
      showAlert("OperationRequest.enterBankBranch");
    }
  };
