...
export interface NewOperationRequestPaymentMethod {
...
const OperationRequestPaymentMethod_VIEW: TextStyle = {
  flex: 1,
...
let isBank = true;
export const NewOperationRequestPaymentMethod: FunctionComponent<NewOperationRequestPaymentMethod> = observer(
  props => {
    ...
    const [PaymentMethodtoPost, setPaymentMethodtoPost] = useState([]);
    ...
    const deleteitem = code => {
      ...
      const removedRcd = PaymentMethodtoPost.filter(item => item.PaymentMethodCode === code);

      onChangeTotPercentage(TotPercentage - parseFloat(removedRcd[1].Percentage));
    ...
    const onAddComm = (PMName, Percentage) => {
      ...
      if (!isExist && isValidPaymentMethod && isValidPercentage) {
        ...
        onChangeTotPercentage(+TotPercentage + parseFloat(Percentage));
        ...
        // PMarr.push({PaymentMethodNameAR:CommName,PaymentMethodCode:CommID,Qty:Qty});
        console.log("PMarr.length: " + PMarr.length);
        console.log(
          "PaymentMethodNameAR: " + PMName + "ID " + PaymentMethodCode + "percentage:" + Percentage,
        );
        console.log("PM arry ", PMarr);
      } else if (!isValidPercentage) {
        showAlert("OperationRequestPaymentMethod.enterPercentage");
      } else if (isExist) {
        showAlert("OperationRequestPaymentMethod.PaymentMethodexisit");
      } else {
        showAlert("OperationRequestPaymentMethod.error");
      }
    };
      ...
      return (
        <View>
          ...
          <View style={FLATList_ITEMS_VIEW}>
            <View style={SUB_CONTAINER}>
              <Text
                tx={"OperationRequestPaymentMethod.Percentage"}
                extraText={":"}
                style={ITEM_LABEL}
                preset={"normal"}
              />
            ...
            >
              <Text tx={"OperationRequestPaymentMethod.delete"} style={[Delete_TEXT_VALUE]} />
            </TouchableOpacity>
          </View>
        </View>
      );
    ...
    async function sumArray(arr: any, key: any) {
      return arr.reduce((a, b) => a + (b[key] || 0), 0);
    }

    const percentsum = function(items, prop) {
    ...
    const onSave = async () => {
      ...
      let ispercentright = false;

      if (TotPercentage === 100) {
        ispercentright = true;
      }
      console.log("OperationRequestStore.isOperationSaved", OperationRequestStore.isOperationSaved);
      if (ispercentright) {
        console.log("JSON.stringify(PaymentMethodtoPost)", JSON.stringify(PaymentMethodtoPost));
        console.log(
          "JSON.stringify(OperationRequestStore.OperationCommodityObj)",
          JSON.stringify(OperationRequestStore.OperationCommodityObj),
        );

        OperationRequestStore.NewOperationRequest(
          OperationRequestStore.OperationObj,
          JSON.stringify(OperationRequestStore.OperationCommodityObj),
          JSON.stringify(PaymentMethodtoPost),
        );

        if (OperationRequestStore.isOperationSaved === false) {
          showAlert("OperationRequest.FailedMsg");
        } else if (OperationRequestStore.isOperationSaved === true) {
          showAlert("OperationRequest.SavedMsg");
          props.navigation.navigate("OperationRequest");
        }
      } else {
        showAlert("OperationRequestPaymentMethod.wrongpercentMsg");
      }
    };
    ...
    const renderView = () => {
      if (!isBank) {
        return (
          <SafeAreaView>
            <ScrollView keyboardShouldPersistTaps="always">
              ...
              <MenuButton
                title={"OperationRequestPaymentMethod.OperationRequestPaymentMethod"}
              ...
              <View style={DETAIL_CONTAINER}>
                ...
                <View style={ITEMS_VIEW}>
                  <View style={TextFiled_VIEW}>
                    <Text tx={"OperationRequestPaymentMethod.Percentage"} style={[ITEM_LABEL]} />
                  </View>
                    ...
                    <TextField
                      ...
                      errorTx={
                        isValidPaymentMethod
                          ? undefined
                          : "OperationRequestPaymentMethod.enterPercentage"
                      }
                  ...
                  >
                    <Text
                      tx={"OperationRequestPaymentMethod.addPaymentMethod"}
                ...
                <View style={BOTTOM_VIEW}>
                  <BottomButton
                    ...
                    leftDisabled={isBank}
                    leftText={"OperationRequestPaymentMethod.send"}
                    rightText={"OperationRequestPaymentMethod.back"}
                    onRightPress={goBack}
        ...
        return (
          <ScrollView>
            <View style={DETAIL_CONTAINER}>
              <View style={OperationRequestPaymentMethod_VIEW}></View>
            </View>
          </ScrollView>
        );
      } else {
