...
export interface UpdateOperationRequestCommodity {
...
let isBank = true;
export const UpdateOperationRequestCommodity: FunctionComponent<UpdateOperationRequestCommodity> = observer(
  props => {
    const isFocused = useIsFocused();
    const { OperationRequestStore, authStore, LookupStore } = useStores();

    const OperationRequestCommodity = OperationRequestStore.getCommodityListData;

    let CommodityArry = [];

    let i = 0;
    for (i = 0; i < OperationRequestCommodity.length; i++) {
      let statusIsPresent = _.indexOf(
        CommodityArry,
        OperationRequestCommodity[i].CommodityNameAR[0],
        0,
      );
      if (statusIsPresent == -1) {
        CommodityArry.push({
          CommodityNameAR: OperationRequestCommodity[i].CommodityNameAR[0],
          CommodityCode: OperationRequestCommodity[i].CommodityCode[0],
          Qty: OperationRequestCommodity[i].Qty[0],
          UnitPrice: OperationRequestCommodity[i].UnitPrice[0],
          InitialInvoiceNumber: OperationRequestCommodity[i].InitialInvoiceNumber[0],
          InitialInvoiceIssuer: OperationRequestCommodity[i].InitialInvoiceIssuer[0],
          InitialInvoiceDate: OperationRequestCommodity[i].InitialInvoiceDate[0],
          CommodityDesc: OperationRequestCommodity[i].CommodityDesc[0],
        });
      }
    }
    ...
    const onSave = async () => {
      ...
      if (CommoditytoPost.length > 0) {
        OperationRequestStore.setUpdateOperationCommodityObj(CommoditytoPost);

        props.navigation.navigate("UpdateOperationRequestPaymentMethod");
      } else {
