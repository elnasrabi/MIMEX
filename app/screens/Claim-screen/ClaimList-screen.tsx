...
import { ComClaimList } from "../../components/Forms/com-Claim-list";
...
export const ClaimList: FunctionComponent<ClaimListProps> = observer(props => {
  const { ClaimStore } = useStores();

  const goBack = React.useMemo(() => () => props.navigation.goBack(), [props.navigation]);

  const goToDetailScreen = detail => {
    const ClaimDetail = detail;
  ...
  return (
    <Screen
      ...
      preset="fixed"
    >
      <BackButton title={"ClaimList.Claims"} onPress={goBack} />

      <FlatList
        style={FLAT_LIST_CONTAINER}
        data={ClaimStore.ClaimList}
        renderItem={({ item, index }) => (
          ...
          >
            <ComClaimList item={item} index={index} />
          </TouchableOpacity>
        )}
