...
export interface UpdateContractRequestCommodity {
...
const ContractRequestCommodity_VIEW: TextStyle = {
  flex: 1,
...
let isBank = true;
export const UpdateContractRequestCommodity: FunctionComponent<UpdateContractRequestCommodity> = observer(
  props => {
    const isFocused = useIsFocused();
    const { ContractRequestStore, authStore, LookupStore } = useStores();
    const ContractRequestCommodity = ContractRequestStore.getCommodityListData;

    let CommodityArry = [];

    let i = 0;
    for (i = 0; i < ContractRequestCommodity.length; i++) {
      let statusIsPresent = _.indexOf(
        CommodityArry,
        ContractRequestCommodity[i].CommodityNameAR[0],
        0,
      );
      if (statusIsPresent == -1) {
        CommodityArry.push({
          CommodityNameAR: ContractRequestCommodity[i].CommodityNameAR[0],
          CommodityCode: ContractRequestCommodity[i].CommodityCode[0],
          Qty: ContractRequestCommodity[i].Qty[0],
          UnitPrice: ContractRequestCommodity[i].UnitPrice[0],
        });
      }
    }
    ...
    const [initialCommodity, changeinits] = useState(CommodityArry);
    const [CommoditytoPost, setCommoditytoPost] = useState(CommodityArry);
    const [CommodityList, setCommodityList] = useState(CommodityArry);
    ...
    const onAddComm = (CommName, QTY, UnitPrice) => {
      ...
      if (!isExist && isValidCommodity && isValidQuantity && isValidUnitPrice) {
        ...
        showAlert("ContractRequestCommodity.enterunitprice");
      } else if (!isValidQuantity) {
        showAlert("ContractRequestCommodity.enterqty");
      } else if (isExist) {
        showAlert("ContractRequestCommodity.commodityexisit");
      } else {
        showAlert("ContractRequestCommodity.error");
      }
    };
      ...
      return (
        <View>
          ...
          <View style={FLATList_ITEMS_VIEW}>
            <View style={SUB_CONTAINER}>
              <Text
                tx={"ContractRequestCommodity.QTY"}
                extraText={":"}
                style={ITEM_LABEL}
                preset={"normal"}
              />
            ...
            <View style={SUB_CONTAINER}>
              <Text
                tx={"ContractRequestCommodity.UnitPrice"}
                extraText={":"}
                style={ITEM_LABEL}
                preset={"normal"}
              />
            ...
            <TouchableOpacity style={TextFiled_VIEW} onPress={() => deleteitem(item.CommodityCode)}>
              <Text tx={"ContractRequestCommodity.delete"} style={[Delete_TEXT_VALUE]} />
            </TouchableOpacity>
          </View>
        </View>
      );
    ...
    const onSave = async () => {
      ...
      if (CommoditytoPost.length > 0) {
        ContractRequestStore.setUpdateContractCommodityObj(CommoditytoPost);

        props.navigation.navigate("UpdateContractRequestPaymentMethod");
      } else {
        showAlert("ContractRequestCommodity.nocommodity");
      }
    };
    ...
    const renderView = () => {
      if (!isBank) {
              ...
              <View style={DETAIL_CONTAINER}>
                ...
                <View style={ITEMS_VIEW}>
                  <TextField
                    ...
                    errorTx={isValidCommodity ? undefined : "ContractRequestCommodity.enterqty"}
                    onChangeText={text => onChangeQuantity(text)}
                    value={Quantity}
                  />
                  <View style={TextFiled_VIEW}>
                    <Text tx={"ContractRequestCommodity.UnitPrice"} style={[ITEM_LABEL]} />
                  </View>
                  ...
                  <View style={ITEMS_VIEW}>
                    <TextField
                      inputStyle={TEXT_VALUE}
                      errorTx={
                        isValidCommodity ? undefined : "ContractRequestCommodity.enterunitprice"
                      }
                  ...
                  >
                    <Text
                      tx={"ContractRequestCommodity.addcommodity"}
                ...
                <View style={BOTTOM_VIEW}>
                  <BottomButton
                    ...
                    leftDisabled={isBank}
                    leftText={"ContractRequestCommodity.continue"}
                    rightText={"ContractRequestCommodity.back"}
                    onRightPress={goBack}
        ...
        return (
          <ScrollView>
            <View style={DETAIL_CONTAINER}>
              <View style={ContractRequestCommodity_VIEW}></View>
            </View>
          </ScrollView>
        );
      } else {
