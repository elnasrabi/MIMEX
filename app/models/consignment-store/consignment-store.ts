...
export const ConsignmentStoreModel = types
  ...
  .actions(self => ({
    ...
    consignmentSearch: flow(function* consignmentSearch(
    ...
    saveConsignment: flow(function* saveConsignment(
      authorization: string,
      consignmentRequest: any,
    ) {
      self.isButtonLoading = true;
    ...
    saveConsignmentOffline: flow(function* saveConsignmentOffline(
      authorization: string,
      consignmentRequest: any,
      id: string,
    ) {
      ...
      try {
        ...
        if (data.kind === "ok" && data.Status === 200) {
          ...
          if (data.location.status === "OK") {
            self.city =
              response.address_components[response.address_components.length - 5].long_name;
            self.district =
